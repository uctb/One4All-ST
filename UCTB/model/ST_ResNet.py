...
class ST_ResNet(BaseModel):
    ...
    def __init__(self,
        ...
        self._num_residual_unit_last = num_residual_unit_last
    ...
    def build(self):
        with self._graph.as_default():
            ...
            x = tf.layers.conv2d(x, filters=self._conv_filters,
                                                  kernel_size=[self._kernel_size, self._kernel_size],
                                                  padding='SAME', activation=tf.nn.relu)
            for i in range(self._num_residual_unit_last):
                x = residual_unit(x)
            # external dims
