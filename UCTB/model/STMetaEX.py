...
class STMetaEX(BaseModel):
    ...
    def __init__(self,
        ...
        super(STMetaEX, self).__init__(code_version=code_version, model_dir=model_dir, gpu_device=gpu_device)
        ...
        self._num_ex_hidden_units = num_ex_hidden_units
    ...
    def build(self, init_vars=True, max_to_keep=5):
        with self._graph.as_default():
            ...
            if self._external_dim is not None and self._external_dim > 0:
                external_input = tf.placeholder(tf.float32, [None, self._external_dim],name='external_feature')
                self._input['external_feature'] = external_input.name
                external_dense = tf.keras.layers.Dense(units=self._num_ex_hidden_units)(external_input)
                external_dense = tf.tile(tf.reshape(external_dense, [1, self._num_node, 1, self._num_ex_hidden_units]),[tf.shape(dense_inputs)[0],1,tf.shape(dense_inputs)[2],1])
        ...
        super(STMetaEX, self).build(init_vars, max_to_keep)
