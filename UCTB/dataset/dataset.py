...
class DataSet(object):
    ...
    def __init__(self, dataset, MergeIndex, MergeWay, city=None, data_dir=None,**kwargs):
        ...
        if self.MergeIndex > 1:
            ...
            if len(self.data['ExternalFeature']['Weather']) > 0:
                self.data['ExternalFeature']['Weather'] = self.merge_data(self.data['ExternalFeature']['Weather'],
                                                                          "node")
        if 'is_padding' in kwargs.keys():
            if kwargs['is_padding']:
                print('padding!')
                self.padding(kwargs['target_width'],kwargs['target_height'])
    ...
    def merge_data(self, data, dataType):
        ...
        if dataType.lower() == "node":
            ...
            for new_ind, ind in enumerate(range(0, data.shape[0], self.MergeIndex)):
                new[new_ind, :, :] = func(data[ind:ind + self.MergeIndex, :, :], axis=0)
        return new
    def padding(self,target_width,target_height):
        data = self.data['Grid']['TrafficGrid']
        t_len,height,width = data.shape
        new_data = np.zeros([t_len,target_height,target_width])
        new_data[:,:height,:width] = data
        self.data['Grid']['TrafficGrid'] = new_data
    def merge_grid(self,scale):
        print('merge!')
        t,h,w = self.grid_traffic.shape
        h_new = h//scale
        w_new = w//scale
        new_grid = np.empty([t,h_new,w_new])
        for i in range(h_new):
            for j in range(w_new):
                new_grid[:,i,j] = np.sum(np.sum(self.grid_traffic[:,i*scale:(i+1)*scale,j*scale:(j+1)*scale],axis=1),axis=1)
        self.grid_traffic = new_grid
        print(self.grid_traffic.shape)