...
class GridTrafficLoader(object):
    ...
    def __init__(self,
                 ...
                 MergeIndex=1,
                 MergeWay="sum",**kwargs):
        self.dataset = DataSet(dataset, MergeIndex, MergeWay, city,data_dir=data_dir,**kwargs)
        ...
        if 'merge_scale' in kwargs.keys():
            self.dataset.merge_grid(kwargs['merge_scale']) 
...
class MultiLevelGridTrafficLoader(object):
    ...
    def __init__(self,
                 ...
                 MergeIndex=1,
                 multi_norm=True,
                 MergeWay="sum",scale_list=None,**kwargs):
        print(kwargs)
        ...
        self.train_data, self.test_data = SplitData.split_data(self.traffic_data, train_test_ratio)
        self.train_ef, self.test_ef = SplitData.split_data(external_feature, train_test_ratio)
        multi_level_train_data = [self.train_data]
        multi_level_test_data = [self.test_data]
        def grid_merge(traffic_grid,scale):
            t,h,w = traffic_grid.shape
            h_new = h//scale
            w_new = w//scale
            traffic_grid_up = np.empty([t,h_new,w_new])
            for i in range(h_new):
                for j in range(w_new):
                    traffic_grid_up[:,i,j] = np.sum(np.sum(traffic_grid[:,i*scale:(i+1)*scale,j*scale:(j+1)*scale],axis=1),axis=1)
            return traffic_grid_up
        max_of_dataset = 0
        min_of_dataset = np.inf
        for scale in scale_list:
            train_grid = multi_level_train_data[-1]
            train_tmp_result = grid_merge(train_grid,scale)
            if max_of_dataset < np.max(train_tmp_result):
                max_of_dataset = np.max(train_tmp_result)
            if min_of_dataset > np.min(train_tmp_result):
                min_of_dataset = np.min(train_tmp_result)
            multi_level_train_data.append(train_tmp_result)
            test_grid = multi_level_test_data[-1]
            test_tmp_result = grid_merge(test_grid,scale)
            multi_level_test_data.append(test_tmp_result)
        # Normalize the traffic data
        multi_level_train_data_normed = []
        multi_level_test_data_normed = []
        normalizers = []
        first = True
        for train_grid,test_grid in zip(multi_level_train_data,multi_level_test_data):
            if multi_norm:
                normalizer = chooseNormalizer(normalize,train_grid)
                normalizers.append(normalizer)
            
                if first:
                    self.train_data = normalizer.transform(train_grid)
                    self.test_data = normalizer.transform(test_grid)
                    first = False

                multi_level_train_data_normed.append(normalizer.transform(train_grid))
                multi_level_test_data_normed.append(normalizer.transform(test_grid))
                self.normalizers = normalizers
            else:
                if first:
                    self.train_data = (train_grid-min_of_dataset)/(max_of_dataset-min_of_dataset)
                    self.test_data = (test_grid-min_of_dataset)/(max_of_dataset-min_of_dataset)
                    first = False
                multi_level_train_data_normed.append((train_grid-min_of_dataset)/(max_of_dataset-min_of_dataset))
                multi_level_test_data_normed.append((test_grid-min_of_dataset)/(max_of_dataset-min_of_dataset))
                self.max_of_dataset = max_of_dataset
                self.min_of_dataset = min_of_dataset
        ...
        for i in range(len(scale_list)+1):
            multi_level_test_data_normed[i] = np.vstack([multi_level_train_data_normed[i][expand_start_index:],multi_level_test_data_normed[i]])
        self.test_data = multi_level_test_data_normed[0]
        self.test_ef = np.vstack([self.train_ef[expand_start_index:], self.test_ef])
        ...
        self.test_period = self.test_period.squeeze(-1)
        self.test_trend = self.test_trend.squeeze(-1)
        self.multi_level_truth_test = [self.test_y]
        self.multi_level_truth_train = [self.train_y]
        for i in range(len(self.scale_list)):
            _,_,_,truth_train = self.st_move_sample.move_sample(multi_level_train_data_normed[i+1])
            _,_,_,truth_test = self.st_move_sample.move_sample(multi_level_test_data_normed[i+1])
            self.multi_level_truth_test.append(truth_test)
            self.multi_level_truth_train.append(truth_train)
